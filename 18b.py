...
for num_obs in range(len(lista_obstaculos)+1):
    if dist_to_target(lista_obstaculos[:num_obs]) == -1:
        solucion = lista_obstaculos[num_obs-1]
        print(','.join([str(x) for x in solucion]))
        break
